...
class Base:
    ...
    def _build(self):
        ...
        self._buildData(result, weatherList)
        self._buildNet()
    ...
    def _buildNet(self):
        ...
        if self._methodId == 0:
            ...
            self._method.add(keras.layers.Embedding(input_dim=len(self._values), output_dim=self._NetTree[0][1], input_shape=(len(self._values[0]),)))
            self._method.add(keras.layers.SimpleRNN(0))
            self._method.add(keras.layers.Dense(9, activation='softmax'))
        self._method.compile(optimizer=self._optimizer, loss=self._loss, metrics=self._metric)
        self._method.summary()

    def _buildData(self, data, weatherList):
        tempWeatherList = []
        for item in weatherList.values():
        ...
        for i in range(len(self._labels)):
            try:
                ...
                if temp.isdigit():
                    ...
                    self._labels[i] = weatherList[self._labels[i]]
            except:
                self._labels[i] = "Нет"
        for i in range(len(self._labels)):
        ...
        print(self._values[:100], self._labels[:100])
        if self._methodId == 0:
        ...
        print(len(self._train_values), len(self._train_labels), len(self._test_values), len(self._test_labels))
# -------------------------------  
...
# SETTERS
    ...
    def _importNet(self):
        ...
        self._isModelUploaded = True

# ADDITIONAL

    def _save(self):
        self._method.save('~/Projects/SimpleNet/SimpleAnomalyNeuralNet/Saves/')
...
# SETTERS
    ...
    def _saveNet(self, name=''):
        if name == name:
            self._method.save('../models/' + name)
        else:
            n = str(date.today())
            t = ttime.localtime()
            t = ttime.strftime("%M%S", t)
            self._method.save('../models/' + n + t)
